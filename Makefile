...
build:
	$(XCODEBUILD) $(XCODE_FLAGS) -configuration $(CONFIGURATION) build

run: build
	open "$(DERIVED_DATA_DIR)/Build/Products/$(CONFIGURATION)/DocNest.app"

test:
	$(XCODEBUILD) $(XCODE_FLAGS) $(TEST_DESTINATION_FLAGS) test

test-unit:
	$(XCODEBUILD) $(XCODE_FLAGS) $(TEST_DESTINATION_FLAGS) test -only-testing:DocNestTests

test-ui:
	$(XCODEBUILD) $(XCODE_FLAGS) $(TEST_DESTINATION_FLAGS) test -only-testing:DocNestUITests

build-for-testing:
	$(XCODEBUILD) $(XCODE_FLAGS) $(TEST_DESTINATION_FLAGS) build-for-testing

analyze:
	$(XCODEBUILD) $(XCODE_FLAGS) -configuration $(CONFIGURATION) analyze

release-build:
	$(XCODEBUILD) -project $(PROJECT) -scheme $(SCHEME) -configuration Release -derivedDataPath $(RELEASE_DERIVED_DATA_DIR) build $(RELEASE_OPTIMIZATION_FLAGS)

archive:
	$(XCODEBUILD) -project $(PROJECT) -scheme $(SCHEME) -configuration Release -archivePath $(ARCHIVE_PATH) archive SKIP_INSTALL=NO BUILD_LIBRARY_FOR_DISTRIBUTION=NO $(RELEASE_OPTIMIZATION_FLAGS)
