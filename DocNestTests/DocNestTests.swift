...
final class DocNestTests: XCTestCase {
    ...
    func testPersistedLibraryRestoreClearsRetiredRecentLibrariesKey() throws {
        let tempRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let libraryURL = tempRoot.appendingPathComponent("Remember Without Recents")

        defer {
            DocumentLibraryService.persistLibraryURL(nil)
            UserDefaults.standard.removeObject(forKey: "recentLibraries")
            try? FileManager.default.removeItem(at: tempRoot)
        }
        ...
        UserDefaults.standard.set(Data([0, 1, 2]), forKey: "recentLibraries")
        let accessSession = try XCTUnwrap(DocumentLibraryService.restorePersistedLibraryAccess())
        defer {
        ...
        XCTAssertEqual(accessSession.url, createdLibraryURL.standardizedFileURL)
        XCTAssertNil(UserDefaults.standard.object(forKey: "recentLibraries"))
    }
